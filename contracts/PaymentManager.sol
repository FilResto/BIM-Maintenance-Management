...
contract PaymentManager {
    address public immutable owner; // Admin or dApp owner
    address public assetManager; // The AssetManager contract address

    event AssetManagerChanged(address indexed previous, address indexed current);
    event TechnicianPaid(address indexed technician, uint256 amountWei);
    event UserReimbursed(address indexed user, uint256 amountWei);
    ...
    function setAssetManager(address newassetManager) external onlyOwner {
        require(newassetManager != address(0), "zero addr");
        emit AssetManagerChanged(assetManager, newassetManager);
    ...
    function payTechnician(
        ...
        require(technician != address(0), "zero addr");
        require(
