...
import { globalId } from "three/tsl";
...
function IFCViewer({ onObjectSelected, userRole, focusGlobalId }) {
  ...
  const focusOnGlobalId = (gid) => {
    ...
    console.log("GlobalId on IFC Viewer: ", gid);
    console.log("ExpressId on IFC Viewer: ", expressId);

    /* B)  Find the fragment that contains that ExpressID           */
    const fragment = model.items.find((f) =>
     (f.ids instanceof Set ? f.ids.has(expressId) : f.ids.includes(expressId))
    );
    if (!fragment) {
    ...
    /* C)  Locate the specific *instance* inside an InstancedMesh   */
  ...
  const handleDoubleClick = async (event) => {
    ...
    const instanceIndex = hits[0].instanceId ?? 0;          // which copy was hit
    const idsArr = Array.from(fragment.ids);         // all ExpressIDs
    const expressId = idsArr[instanceIndex];            // the matching one
  ...
  useEffect(() => {
    (async () => {
      ...
      const response = await fetch("/testModello.ifc");
